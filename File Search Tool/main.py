...
st.title("Gemini File Search Tool")
st.write(
    "Upload a **.txt** or **.pdf** file, index it with Gemini File Search, "
...
uploaded_file = st.sidebar.file_uploader(
    "Upload a .txt or .pdf file", type=["txt", "pdf"], accept_multiple_files=False
)
...
file_display_name = st.sidebar.text_input(
    "File Display Name (for citations)", value="uploaded-file"
)
...
if create_store:
    if not uploaded_file:
        ...
        try:
            ...
            uploaded_extension = os.path.splitext(uploaded_file.name)[1]  
            if uploaded_extension.lower() not in [".txt", ".pdf"]:
                uploaded_extension = ".txt"
            
            with tempfile.NamedTemporaryFile(delete=False, suffix=uploaded_extension) as tmp:
